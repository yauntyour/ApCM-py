...
if __name__ == "__main__":
    ...
    logging.info("=" * 60)
    logging.info("🎯 训练策略：使用真实数据训练，真实数据测试")
    logging.info("=" * 60)
    # ==================== 加载真实测试数据 ====================
    logging.info("\n📂 加载真实数据...")
    try:
    ...
    # 预处理测试数据
    train_data = preprocess_images(test_images_raw, target_size=(32, 32), is_flat=False)
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 50 == 0:
            logging.info(
                f"Epoch {epoch+1}/{epochs}, Loss: {loss.item():.6f}, Recon Loss: {recon_loss.item():.6f}, LR: {current_lr}"
            )
    ...
    plot_loss_curves(
        train_loss_history,
        train_recon_loss_history,
        save_path="assets/loss_curves_res_"
        + str(input_dim // m_dim)
        + "Lr["
